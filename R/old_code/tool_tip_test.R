...
server <- function(input, output) {
  ...
  output$hover_box <- renderPrint ({
    nearPoints(mtcars, input$plot_click, threshold = 100, maxpoints = 1, addDist=TRUE)  
               #xvar='mpg', yvar='hp')
    # hover <- input$plot_click
    # point <- nearPoints(mtcars, hover, threshold = 100, maxpoints = 1,  
    #                     xvar='mpg', yvar='hp')
    ...
    # 
    # cat(paste('nrow(point) = ',nrow(point),"\n",
    #       'top_pct = ', top_pct, "\n", 
    #       'left_px = ', left_px, "\n",
    #       'top_px = ', top_px, "\n"
    #       ))  
  ...
  output$hover_info <- renderUI({
    ...
    point <- nearPoints(mtcars, input$plot_hover, threshold = 10, maxpoints = 1, addDist = TRUE, 
                        xvar='mpg', yvar='hp')
    #if (nrow(point) == 0) return(NULL)
